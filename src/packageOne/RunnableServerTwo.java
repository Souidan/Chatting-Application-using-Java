...
public class RunnableServerTwo implements Runnable {
	Socket connectionsocket;
	Socket masterConnectionSockcet;

	public RunnableServerTwo(Socket s) throws UnknownHostException, IOException {
		connectionsocket = s;
		...
		try {
			...
			while (true) {
				...
				if (clientSentence.length() >= 11
					...
					outToMaster.writeBytes("FromSlaveRequest.1."
							+ clientSentence.substring(12,
									clientSentence.length()) + "\n");
					String infromM = inFromMaster.readLine();
					System.out.println("testtt33");
					if (infromM.equals("user registered")) {
						TCPServer2.users.add(clientSentence.substring(12,
								clientSentence.length()));
						System.out.println(TCPServer2.users.get(0));
						TCPServer2.usersockets.add(connectionsocket);
						outToClient
					...
					if (clientSentence.equals("Server/GetList")) {
						...
						if (clientSentence.equals("LOGOFFALLSERVERS")) {
							int index = TCPServer2.usersockets
									.indexOf(connectionsocket);
							String name = TCPServer2.users.get(index);
							outToMaster.writeBytes("LOGOFFALLSERVERSSLAVE."
									+ name + "\n");
							TCPServer2.users.remove(index);
							TCPServer2.usersockets.remove(index);
							...
							if (clientSentence.length() >= 17
								...
								int indexdest = TCPServer2.users.indexOf(dest);
								Socket socketdest = TCPServer2.usersockets
										.get(indexdest);
								...
								dest = message[1];
								text = message[2];
								int indexsource = TCPServer2.users
										.indexOf(source);
								int indexdest = TCPServer2.users.indexOf(dest);
								if (indexdest == -1) {
									outToMaster
											.writeBytes("FromSlaveMessage.1."
													+ source + "." + dest + "."
													+ text + "\n");
									...
									Socket to = TCPServer2.usersockets
											.get(indexdest);
									DataOutputStream outTo = new DataOutputStream(
											to.getOutputStream());
